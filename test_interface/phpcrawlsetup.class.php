...
class PhpcrawlSetup extends PHPCrawler
{
  ...
  {
    #foreach(array_keys($this->setup_array_raw) as $key)
    foreach($this->setup_array_raw as $key => $arr_val)
    {
      if ($key=="setURL") {
      ...
      if ($key=="setTrafficLimit") {
        $value = (int)$this->setup_array_raw[$key] * 1024;
        $this->setup_array[$key]["arg_type"] = "integer";
  ...
  function setupCrawler () {
    ...
    if (isset($this->output_array) && count($this->output_array)==0) {
      $setup_error = "No output specified, please choose some output.";
    }
    else {

      foreach($this->setup_array as $key => $arr_val)
      {
  ...
  function setObjectMethods ($method, $arguments, $str_argument_type) {
    ...
    for ($x=0; $x<count($arguments); $x++) {
      ...
      if (is_array($arguments[$x])) {
        ...
        foreach($arguments[$x] as $key => $arr_val) {
  ...
  function handleDocumentInfo($DocInfo) {
    ...
    foreach($this->output_array as $key => $arr_val)
    {
      ...
      if ($key=="content_size")
      {
        ...
        if (!isset($str) || ( isset($str) && trim($str)=="")) { $str = "??"; };
