...
class PHPCrawlerLinkFinder
{
  ...
  public function findLinksInHTMLChunk(&$html_source)
  {
    ...
    for ($x=0; $x<$cnt; $x++)
    {  
      $link_raw = isset($matches[1][$x]) ? trim($matches[1][$x]) : "";
      $linktext = isset($matches[2][$x]) ? $matches[2][$x] : "" ;
      $linkcode = isset($matches[0][$x]) ? trim($matches[0][$x]) : "";
    ...
    if ($this->aggressive_search == true)
    {
      ...
      for ($x=0; $x<count($pregs); $x++)
      {
        ...
        for ($y=0; $y<$cnt; $y++)
        {
          $link_raw = isset($matches[2][$y]) ? $matches[2][$y] : "" ;
          $linkcode = isset($matches[0][$y]) ? trim($matches[0][$y]) : "";
          $linktext = "";
