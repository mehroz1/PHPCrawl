  ...
  public static function getFromHeaderLine($header_line, $source_url)
  {
    ...
    for ($x=1; $x<count($parts); $x++)
    {
      ...
      if (preg_match("#^expires\s*=(.*)# i", $parts[$x], $match)) {
          $expires = isset($match[1]) ? trim($match[1]) : "";
          unset($match);
      }
      if (preg_match("#^path\s*=(.*)# i", $parts[$x], $match)) {
          $path = isset($match[1]) ? trim($match[1]) : "";
          unset($match);
      }
      if (preg_match("#^domain\s*=(.*)# i", $parts[$x], $match)) {
          $domain = isset($match[1]) ? trim($match[1]) : "";
          unset($match);
      }
    }
    unset($parts);

    $expires = str_replace("\"", "", (isset($expires) ? $expires : "") );
    $path = str_replace("\"", "", (isset($path) ? $path : "") );
    $domain = str_replace("\"", "", (isset($domain) ? $domain : "") );
